...
**5. High capital tax rates are ineffective when enforcement is weak.** Instrument design matters more than statutory rate. A 50% rate with 30% enforcement generates less revenue than a 20% rate at full enforcement. Enforcement-immune instruments — land value taxes, energy levies, value-added taxes — outperform capital income taxes for small and medium nations.

**6. Concentrated ownership erodes the institutions that would correct it.** In the governance decay extension of the model, concentrated ownership raises inequality, which reduces enforcement capacity, which lowers effective redistribution, which raises inequality further. By t+20 under concentrated low-tax scenarios, the model's institutional capacity index falls to ~0.48 — a statutory 50% rate effectively collects around 10%.

**7. The yeoman model requires open commerce infrastructure.** Without a federated open marketplace making agent-to-agent contracting and automatic tax reporting available at near-zero cost, small operators cannot participate in machine-speed commerce and the platform layer reintroduces the same rent extraction it was meant to replace. This infrastructure is also a national security instrument: it is the mechanism that makes foreign piecemeal asset accumulation and knowledge-work offshoring legible to regulators.
