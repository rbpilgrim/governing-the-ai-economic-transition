...
Within the conditional frame, the pattern that keeps reappearing across scenario configurations is that **distributional outcomes appear to depend primarily on who owns the AI capital — and the policy instruments that perform well are different from the ones we would reach for by default.** Seven structural patterns follow. First, the nominal economy grows rather than collapses: AI-driven deflation in knowledge work and services is offset by Jevons-paradox dynamics in physical goods, a two-phase energy transition, and appreciating scarcity goods. The problem is composition, not size. Second, the fiscal problem is therefore not insolvency but instrument mismatch: the tax base is shifting from labour income (high-enforcement, withholding-based) to capital income (mobile, leakage-prone). Third, in this model direct ownership of AI capital consistently outperforms equivalent ex-post fiscal redistribution on distributional outcomes — by roughly 2-to-1 on inequality reduction across most parameterisations, driven by the marginal propensity to consume differential between concentrated capital income (~25%) and earned income (~78%). This result is robust to wide variation in parameters but depends on the yeoman equilibrium being supported, not emerging spontaneously from markets. Fourth, distributed-ownership models have a structural fiscal prerequisite that does not exist today: the current tax code imposes an ~18% effective income discount on independent operators, representing roughly $2.4 trillion per year in foregone income at full deployment. Fifth, high statutory capital tax rates are ineffective when capital is mobile and enforcement weak; instrument design matters more than rate. Sixth, concentrated ownership erodes the institutions required to collect the taxes that would otherwise correct it, producing a self-reinforcing dynamic whose severity is understated by static analysis. Seventh, the yeoman model requires not only fiscal reform but commerce infrastructure: without a federated open marketplace that makes agent-to-agent contracting and automatic tax reporting available at near-zero transaction cost, small operators cannot participate in machine-speed commerce and the Coasian fragmentation dynamic stalls at the point of platform capture.

The contribution is not the specific numbers — they will be wrong in detail — but the structural relationships between ownership, instrument choice, and outcome that the model makes visible, and which recur across the sensitivity range tested here. This paper is offered as a conversation starter about how we might respond if rapid AI takeoff arrives faster than the institutions that would govern it.
...
We compensate for the model's unreliability by running an unusually wide sensitivity analysis: ten independently varying policy dimensions, automation speeds from 5 to 14 years to midpoint, statutory capital tax rates from 20% to 50%, enforcement levels from 30% to 100%, and ownership structures spanning concentrated, distributed-yeomen, commons-DAO, and public-infrastructure models. The point is not that any single combination predicts what will happen. The point is that across the full range of settings we tested, the same structural relationships keep appearing, and the same instruments keep working or failing for the same reasons. The conversation starter we are offering is that pattern, not the numbers attached to any specific run.

We acknowledge directly what the model cannot do. It cannot anticipate how the system will adapt — how households will reorganise consumption, how firms will reorganise themselves around AI-mediated coordination, how political coalitions will form, how new institutions will emerge or fail. These adaptations will dominate the actual trajectory. Treating the model as a forecast would be a category error. Treating it as a way to organise an argument about which policy instruments are structurally robust to the kind of change rapid automation would represent — that is what it is for.
