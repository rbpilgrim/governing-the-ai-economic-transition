...
**The asset-pricing channel: limited capital market participation** (Maresca, 2026; Moll et al., 2022). Most households are institutionally restricted to safe-asset positions: pension reserves under liability-driven mandates, bank deposits, insurance reserves subject to solvency rules. In Maresca's heterogeneous-agent model, only a small fraction (χ ≈ 0.10) of households directly hold productive risky capital; the remainder are "Savers" whose only response to disappearing wage income is precautionary accumulation of bonds and cash. As automation destroys human wealth, this precautionary demand drives the risk-free rate to its cash floor while the equity premium expands toward 20%+, and the Pareto tail of the wealth distribution compresses toward 1.0 (Zipf's Law). This is the asset-side mirror of the demand-side mechanism the present paper models. The yeomen policy is, in this language, the policy of raising χ — the fraction of households with a direct claim on productive capital — from roughly 0.1 toward 1.0. The MPC differential we exploit on the demand side and the wealth-condensation Maresca derives on the asset side are the same phenomenon viewed from two angles.

**The scarcity-goods question: what we are *not* relying on.** Trammell (2025) argues that even with non-satiated preferences and a willingness to pay a premium for human-authored goods, the labour share of income still trends toward zero in the long run: multiple non-human categories (absolutely scarce resources, contest goods, slow-satiation R&D) compete for the rising-luxury slot, the opportunity cost of consuming inferior human-made alternatives rises as capital-produced quality improves, and continuous invention raises the utility ceiling above human-made goods. Empirically, arts and entertainment is approximately 1.1% of US GDP today; the "scarcity premium" has not historically elevated such sectors to macroeconomic prominence. Our model includes a scarcity-goods sector for completeness, but the central argument does *not* depend on this sector becoming load-bearing. If anything, Trammell's argument strengthens the case for distributed ownership: if natural preference structures will not preserve broad-based labour income, then ownership of the capital displacing labour becomes the only durable path to broad-based participation. We explicitly do not make a Baumol-rescue argument.

**The optimistic counterfactual and why it does not close.** Albrecht (2026) is the principal contrary view: the horses-and-tractors analogy fails for humans because saved dollars historically migrate toward services and other human-intensive uses (Hicks–Marshall scale effects, income-elastic structural change, multiple comparative advantages, relational preferences). The argument has a real historical track record but a structural weakness that Trammell's enumeration exposes directly. Albrecht's mechanism requires saved dollars to *land on* human labour, not merely to be reallocated. Trammell shows the reallocation channel is wide and agnostic about its destination: absolutely scarce resources (land, minerals, historical artefacts), contest goods (military equipment, legal services, marketing), and slow-satiation goods (R&D, life extension, philanthropy, status competition) all compete for the rising-luxury slot. There is no economic law forcing the displaced labour share to be recovered through human-intensive consumption specifically. On the demand side, human-experience goods saturate: there is a ceiling on how many bespoke concerts, live performances, or relational-care hours any one person consumes per year, and that ceiling rises slowly with wealth. The share-equilibrium Albrecht needs requires labour's slice of the reallocation pie to be roughly the slice that sustains today's labour income — a numerical coincidence with no mechanism behind it. The previous waves landed on services because services were the cognitive-and-relational residual; the current wave is targeted at exactly that residual. We therefore do not treat Albrecht's case as a symmetric counterfactual to ours, but as a position whose own internal logic, combined with Trammell's, points in our direction: if natural preference structure will not recover the labour share, ownership of the displacing capital is the only durable channel back to broad participation.
